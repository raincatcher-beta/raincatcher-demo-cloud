...
var $fh, initialized = false, notifications, listeners = [];
...
function init(_$fh, _syncOptions) {
  $fh = _$fh;
  notifications = Rx.Observable.create(function (observer) {
...
DataManager.prototype.create = function(object) {
  ...
  $fh.sync.doCreate(self.datasetId, object, function(msg) {
    ...
    notifications.filter(function(notification) {
      return notification.dataset_id === self.datasetId
        && notification.code == 'local_update_applied'
...
DataManager.prototype.waitForSync = function() {
  ...
  notifications.filter(function(notification) {
    return notification.dataset_id == self.datasetId
      && (notification.code == 'sync_complete' || notification.code == 'sync_failed')
  }).take(1).toPromise(q.Promise)
  .then(function(notification) {
