...
var SimpleStore = require('fh-wfm-simple-store'), //Shall be changed to require('fh-wfm-simple-store')({persistent:true})
...
function initStore(dataSetId, seedData, mediator) {
  ...
  return dataStore.list().then(function(storeData) {
    if (!storeData || storeData.length === 0) {
      ...
      if (!seedData && process.env.WFM_USE_MEMORY_STORE === "true") {
        seedData = [];
      }
