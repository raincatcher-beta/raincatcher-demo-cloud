...
describe("Data Set Hash Functions", function() {
  ...
  describe("Result", function() {
    ...
    var mockHash = hash.sha1(mockResult);

    it("same objects should have same hashes", function() {
      expect(hashFunctions.result(mockResult)).to.equal(mockHash);
    });

    it("different object should have different hashes", function() {
      expect(hashFunctions.result(_.omit(mockResult, 'id'))).to.not.equal(mockHash);
    });

    it("should ignore fields that are not part of a result", function() {
      expect(hashFunctions.result(_.extend({meh: "arg"}, mockResult))).to.equal(mockHash);
    });
