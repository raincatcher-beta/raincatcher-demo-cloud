...
function initStore(dataSetId, seedData, mediator) {
  return db.getCollectionDal(dataSetId).then(function(_dal) {
    return _dal.list().then(function(storeData) {
      //console.log("Initial data",storeData);
      if (!storeData || storeData === null || storeData.length === 0) {
        ...
        return _dal.init(seedData).then(function() {
          return _dal.listen(dataTopicPrefix, mediator);
        });
      } else {
        return _dal.listen(dataTopicPrefix, mediator);
      }
    ...
    console.error('Error: Unable to get Mongoose DAL object for %s', dataSetId);
  });
  
  // if (dataStore && !dataStore.collection) {
  //   console.log('NOT READY', dataStore);
  //   return;
  // } else {
  
  //       console.log('READY', dataStore);
  /**
