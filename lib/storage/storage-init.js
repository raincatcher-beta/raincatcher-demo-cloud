...
var connectPromise, useMongoStore;
 ...
 * @param {Function} customListenHandler custom method that allows developers to override specific crud methods.
 */
function initStore(dataSetId, seedData, mediator, customListenHandler) {
  if (!connectPromise) {
  ...
  return connectPromise.then(function() {
    var store = useMongoStore ? mongoStore : simpleStore;
    ...
    return store.getCollectionStore(dataSetId).then(function checkForInit(dataStore) {
      ...
      if (customListenHandler) {
        return customListenHandler(dataStore, dataTopicPrefix);
      } else {
...
function connect() {
  connectPromise = connectPromise || Promise.fromCallback(function(callback) {
    ...
    if (connectionUrl) {
      useMongoStore = true;
      return mongoStore.connect(connectionUrl);
    } else {
...
function disconnect() {
  ...
  connectPromise.then(function() {
    var promise = useMongoStore ? mongoStore.disconnect() : simpleStore.disconnect();
