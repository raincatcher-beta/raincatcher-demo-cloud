...
describe("Raincatcher Sync Client Initialisation", function() {
  ...
  it("should not allow a manager to be created if sync has not been initialised", function() {
    ...
    return syncClient.manage(mockDataSetId, mockSyncOptions, mockDataSetFilterParams, mockDataSetMetadataParams).then(function(message) {
      expect(message).to.equal('Sync not yet initialized.  Call sync-client.init() first.');
