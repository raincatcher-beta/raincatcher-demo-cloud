...
module.exports = function(mediator) {
  ...
  workorders.forEach(function(workorder, index) {
    ...
    workorder.finishTimestamp = tomorrow.getTime();
  });

  return store.init('workorders', workorders, mediator, function(dataStore, dataTopicPrefix) {
    var parentListFunction = _.bind(dataStore.list, dataStore);
    return dataStore.listen(dataTopicPrefix, mediator, {
      // Override list method for workorders
      list: function(filter) {
        if (filter.sessionToken) {
          return userSessionApi.getUserIdForSession(filter.sessionToken).then(function(userId) {
            // Override filter with exact user id
            filter.value = userId;
            delete filter.sessionToken;
            return parentListFunction(filter);
          });
        } else {
          return parentListFunction(filter);
        }
