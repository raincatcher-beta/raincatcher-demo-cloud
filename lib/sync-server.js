...
function initSync(mediator, mbaasApi, datasetId, syncOptions){
  ...
  var dataListHandler = function(datasetId, queryParams, cb, metaData){
    mediator.request('sync:'+datasetId+':list:load', queryParams, {uid: null, timeout: 5000})
    .then(function(data) {
      var syncData = {};
  ...
  var dataCreateHandler = function(datasetId, data, cb, meta_data) {
    ...
    mediator.request('sync:'+datasetId+':create', [data, ts], {uid: ts})
  ...
  var dataSaveHandler = function(datasetId, uid, data, cb, meta_data) {
    mediator.request('sync:'+datasetId+':save', data, {uid: uid})
    .then(function(object) {
      cb(null, object);
    }, function(error) {
      console.log('Sync error: init:', datasetId, error);
  ...
  var dataGetHandler = function(datasetId, uid, cb, meta_data) {
    mediator.request('sync:'+datasetId+':load', uid)
    .then(function(object) {
      cb(null, object);
    }, function(error) {
      console.log('Sync error: init:', datasetId, error);
  ...
  var dataDeleteHandler = function(datasetId, uid, cb, meta_data) {
    mediator.request('sync:'+datasetId+':delete', uid)
